...
- **Lanes run in the background by default.** The architect launches every lane with `run_in_background: true`, ends its turn with a one-line status, and acts on the completion notification. A sequential dependency is not a reason to hold the turn open for a lane's whole wall clock while the user can't interject — synchronous waits are the rare, announced exception, not the default.
