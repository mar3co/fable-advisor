...
> **Fork notice (mar3co/fable-advisor).** This fork diverges from [upstream](https://github.com/DannyMac180/fable-advisor) doctrinally, on purpose: the default implementation lane is **configurable** — `codex-implementer` (GPT-5.6 Sol, high reasoning) when unconfigured, or `grok-implementer` if any CLAUDE.md that applies to the session declares `fable-advisor: default implementation lane = grok` (one line; clear freeform statements count too). The other CLI lane is the first fallback and a Claude Opus subagent is ALWAYS the final fallback, every step announced; lanes are never raced — assurance comes from cross-vendor review of the diff, not duplicate implementations. It also adds two agents upstream doesn't have: `grok-researcher` (live web/X research + mechanical codebase lookups) and `grok-reviewer` (cold diff-only second review lens). Lane mechanics are kept identical to upstream so merges stay trivial. The tables and prose below are upstream's and describe grok-as-default — where they conflict with this notice, the fork's SKILL.md and agent files win.
