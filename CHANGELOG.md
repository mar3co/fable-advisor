...
- **Wrappers stop-and-report on foreign git state; verification and settle are gated on a rewind-proof stability anchor.** Both `grok-implementer.md` and `codex-implementer.md` capture `BRANCH=$(git branch --show-current)` at baseline alongside `BASELINE`, then re-verify three conditions at verification (step 4) and again at settle (step 5): the current branch is still `$BRANCH`; `$BASELINE` is still an ancestor of HEAD (`git merge-base --is-ancestor $BASELINE HEAD` — a foreign rewind makes `git log $BASELINE..HEAD` deceptively empty, so an empty range is not a pass by itself); and everything in that range is this task's work. Wrong-branch, rewind, or foreign commits are `STATUS: partial` with no mutating self-correction. A Rules bullet bans destructive self-correction of shared git state at every stage of the run.
